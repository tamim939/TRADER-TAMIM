...
export const useSignals = () => {
  ...
  useEffect(() => {
    ...
    const handleSignal = (newSignal: SignalData) => {
      ...
      if (newSignal.signal && signalKey !== lastSignalRef.current) {
        vibrate();
      }
    ...
    const updateSignal = async () => {
      try {
        ...
        // If we have a manual signal for the current period, use it
        if (latest.signal && latest.period === currentPeriod) {
          handleSignal(latest);
        } else if (currentPeriod) {
          // Otherwise, generate an automatic one
          handleSignal({
        ...
        // Fallback to auto signal on error
        if (currentPeriod) {
          handleSignal({
            signal: getAutoSignal(currentPeriod),
            period: currentPeriod
